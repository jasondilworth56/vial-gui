...
class FakeAppctx:
    def get_resource(self, path):
        return os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "../../resources/base/", path
        )
...
def test_about_keyboard(qtbot):
    ...
    assert mw.about_dialog.textarea.toPlainText() == (
...
def test_key_change(qtbot):
    ...
    qtbot.mouseClick(
        mw.keymap_editor.container,
        qt_api.QtCore.Qt.MouseButton.LeftButton,
        pos=QPoint(int(point.x()), int(point.y())),
    )

    assert (
        mw.keymap_editor.container.active_key == mw.keymap_editor.container.widgets[0]
    )
    ...
    assert (
        mw.keymap_editor.container.active_key == mw.keymap_editor.container.widgets[1]
    )
    ...
    def find_key_btn(start, text):
        for w in start.findChildren(SquareButton):
            if w.isVisible() and w.text == text:
                return w
        raise RuntimeError(
            "cannot find a visible key button with text='{}'".format(text)
        )
    ...
    assert (
        mw.keymap_editor.container.active_key == mw.keymap_editor.container.widgets[2]
    )
...
def test_combos(qtbot):
    ...
    mw, vk = prepare(
        qtbot,
        FAKE_KEYBOARD,
        combos=[[0, 0, 0, 0, 0], [4, 5, 6, 7, 8], [0, 0x106, 0, 0, 0]],
    )
    ...
    def check_tab(idx, keys):
        ...
        assert len(w) == 5
        for x in range(5):
            assert w[x].keycode == keys[x], (
                "unexpected keycode at tab {} position {}: {} vs {}".format(
    ...
    qtbot.mouseClick(
        find_key_btn(mw.tray_keycodes, "A"), qt_api.QtCore.Qt.MouseButton.LeftButton
    )
    ...
    qtbot.mouseClick(
        find_key_btn(mw.tray_keycodes, "B"), qt_api.QtCore.Qt.MouseButton.LeftButton
    )
    ...
    assert ak.tabText(ak.currentIndex()) == "Quantum"
    qtbot.mouseClick(
        find_key_btn(mw.tray_keycodes, "LSft\n(kc)"),
        qt_api.QtCore.Qt.MouseButton.LeftButton,
    )
    ...
    assert bk.isVisible()
    qtbot.mouseClick(
        find_key_btn(mw.tray_keycodes, "D"), qt_api.QtCore.Qt.MouseButton.LeftButton
    )
    ...
    qtbot.mouseClick(
        find_key_btn(mw.tray_keycodes, "E"), qt_api.QtCore.Qt.MouseButton.LeftButton
    )
...
def test_tap_dance(qtbot):
    ...
    mw, vk = prepare(
        qtbot,
        FAKE_KEYBOARD,
        tap_dance=[[0, 0, 0, 0, 200], [4, 5, 6, 7, 200], [0, 0x106, 0, 0, 500]],
    )
